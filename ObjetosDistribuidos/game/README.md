...
- **Objetos Distribuidos con RMI**: Broker RMI para invocation de métodos remotos
...
|--------|-------------|-------|
| **Sockets TCP** | Sistema original de comunicación texto/JSON | 12345 (matchmaking), 50001-50100 (juego) |
...
|------------|-------------|
| **MatchmakingServer** | Servidor central en puerto 12345 que gestiona el registro y descubrimiento de partidas (sockets) |
...
| **GameObjectBroker** | Broker RMI en puerto 1099 que gestiona objetos remotos |
| **RemoteMatchmaking** | Interfaz remota que define los métodos RMI disponibles |
...
## ☁️ Objetos Distribuidos con RMI

### Broker RMI (GameObjectBroker.java)
- **Puerto RMI**: 1099
- **Protocolo**: Java RMI (Remote Method Invocation)
- **Funciones**:
  - Crea y gestiona el Registry RMI
  - Registra objetos remotos para acceso remoto
  - Permite invocación de métodos remotos

**Iniciar el Broker:**
```bash
java -cp "target/classes:json-simple-1.1.1.jar:lanterna-3.1.1.jar" backend.rmi.GameObjectBroker
```

### RemoteMatchmaking (Interfaz Remota)
- **Puerto**: 1099
- **Funciones** (invocables remotamente):
  - Crear partidas
...
**Comandos RMI:**
```java
createGame(username, gameName, maxPlayers)  → Crea una nueva partida
listGames()                                    → Lista todas las partidas
joinGame(port)                                 → Se une a una partida
updateGame(port, currentPlayers)              → Actualiza el conteo
deleteGame(port)                               → Elimina una partida
ping()                                         → Verifica conexión
```

### Cliente RMI (RMIClient.java)
Cliente de prueba para verificar el sistema RMI:
```bash
java -cp "target/classes" backend.rmi.RMIClient
```

**Menú del cliente RMI:**
```
1. create <nombre> <max>  - Crear partida
2. list                   - Listar partidas
3. join <puerto>         - Unirse a partida
4. update <puerto> <n>   - Actualizar jugadores
5. delete <puerto>        - Eliminar partida
6. ping                   - Verificar conexión
7. quit                   - Salir
```

### Diferencia entre Sockets y RMI

| Característica | Sockets | RMI |
|----------------|---------|-----|
| **Nivel** | Bajo nivel (bytes/texto) | Alto nivel (objetos Java) |
| **Serialización** | Manual (JSON) | Automática |
| **Invocación** | Envío de mensajes | Llamada a métodos |
| **Puerto** | 12345, 50001-50100 | 1099 |
| **Complejidad** | Mayor (parser JSON) | Menor (objetos remotos) |
| **Acoplamiento** | Menor (protocolo texto) | Mayor (interfaces Java) |
...
│   │           │   ├── GameObjectBroker.java      # Broker RMI
│   │           │   ├── RemoteMatchmaking.java     # Interfaz remota
│   │           │   ├── RemoteMatchmakingImpl.java # Implementación
...
│               └── AppTest.java
├── pom.xml
├── json-simple-1.1.1.jar         # Dependencia JSON
├── lanterna-3.1.1.jar            # Dependencia terminal
└── README.md
```
game/
├── src/
...
O manualmente (si Maven no está configurado):
```bash
javac -cp "json-simple-1.1.1.jar:lanterna-3.1.1.jar" -d target/classes src/main/java/backend/**/*.java
```

### Ejecución del Sistema

#### Sistema Original (Sockets)
...
#### Sistema RMI (Objetos Distribuidos)

##### Paso 1: Iniciar el Broker RMI

```bash
java -cp "target/classes:json-simple-1.1.1.jar:lanterna-3.1.1.jar" backend.rmi.GameObjectBroker
```

El broker escuchará en el puerto 1099.

##### Paso 2: Probar con Cliente RMI (en otra terminal)

```bash
java -cp "target/classes" backend.rmi.RMIClient
```

O conectar desde otra máquina:
```bash
java -cp "target/classes" backend.rmi.RMIClient <ip-del-broker> 1099
```
...
| Comunicación | Sockets TCP (JSON) | Sockets + RMI (objetos remotos) |
| Broker RMI | No disponible | Puerto 1099 |
